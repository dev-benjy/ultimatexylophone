...
namespace AndroidAudioBypass
{
    ...
    public class BypassAudioManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            using (AndroidJavaClass pluginClass = new AndroidJavaClass("com.crosscakeinteractive.androidbypass.AudioBypass"))
            {
        ...
        public int UnregisterSound(int soundId)
        {
            if (audioBypassInstance == null) { return -1; }
            audioBypassInstance.Call<int>("unload", soundId);
            return soundId;
        }
        public float ChangeVolumeRealtime(float volume)
        {
            if (audioBypassInstance == null) { return -1f; }
            audioBypassInstance.Call<float>("StreamVolumeControl", volume);
            return volume;
        }
